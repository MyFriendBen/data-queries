{{ config(
    materialized='view',
    description='Staging model for complete screener data with eligibility calculations'
) }}
...
latest_eligibility_snapshot_by_screen_id as (
    select
        sc.screen_id
        ,(select id
...
monthly_expenses_by_screener_id as (
    select
        screen_id,
        sum(case
           when frequency = 'yearly' then amount / 12
...
latest_program_eligibility as (
    ...
    from {{ source('django_apps', 'screener_programeligibilitysnapshot') }} spes
    where eligible = true
    group by spes.eligibility_snapshot_id
),

household_totals_and_percentages as (
    select
        shou.screen_id
    ...
    from {{ source('django_apps', 'screener_householdmember') }} shou
    left join {{ source('django_apps', 'screener_screen') }} sscr on shou.screen_id = sscr.id
